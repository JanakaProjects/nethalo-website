...
const getStatusStyle = (status: string) => {
  switch (status) {
    ...
    default: return { background: '#f5f5f7', color: 'var(--color-text-secondary)' };
  }
};
  ...
  return (
    <DashboardLayout>
      <div style={{ marginBottom: isMobile ? 24 : 32 }}>
        <h1 style={{ fontSize: isMobile ? 22 : 28, fontWeight: 700, color: 'var(--color-text-primary)', margin: 0, letterSpacing: '-0.02em' }}>Parent Dashboard</h1>
        <p style={{ fontSize: isMobile ? 14 : 16, color: 'var(--color-text-secondary)', marginTop: 6 }}>Monitor your child&apos;s safety at a glance</p>
      </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: isMobile ? '1fr' : '1fr 1fr', gap: isMobile ? 12 : 24, marginBottom: isMobile ? 24 : 32 }}>
        <div style={{ background: '#ffffff', borderRadius: 16, padding: isMobile ? '16px' : '24px 24px 20px', border: '1px solid #e8e8ed' }}>
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 16 }}>
            <div>
              <h3 style={{ fontSize: isMobile ? 15 : 16, fontWeight: 600, color: 'var(--color-text-primary)', margin: '0 0 4px 0' }}>Flagged Interactions</h3>
              <p style={{ fontSize: isMobile ? 13 : 14, color: 'var(--color-text-secondary)', margin: 0 }}>Threats detected this week</p>
            </div>
          ...
          {barData.length > 0 ? (
            ...
            <div style={{ fontSize: 14, color: 'var(--color-text-secondary)', textAlign: 'center', padding: 20 }}>No data this week</div>
          )}
        </div>

        <div style={{ background: '#ffffff', borderRadius: 16, padding: isMobile ? '16px' : '24px', border: '1px solid #e8e8ed' }}>
          <h3 style={{ fontSize: isMobile ? 15 : 16, fontWeight: 600, color: 'var(--color-text-primary)', margin: '0 0 16px 0' }}>Recent Updates</h3>
            ...
            {[
              ...
              <div key={i} style={{ display: 'flex', gap: 12, alignItems: 'flex-start' }}>
                ...
                <div style={{ flex: 1 }}>
                  ...
                  <div style={{ fontSize: 11, color: 'var(--color-text-secondary)', marginTop: 2 }}>{item.time}</div>
                </div>
              </div>
            ))}
          </div>
      ...
      <div style={{ background: '#ffffff', borderRadius: 16, padding: isMobile ? '16px' : '24px', border: '1px solid #e8e8ed' }}>
        <h3 style={{ fontSize: isMobile ? 15 : 16, fontWeight: 600, color: 'var(--color-text-primary)', margin: '0 0 12px 0' }}>Connected Children</h3>
        <div style={{ display: 'flex', flexDirection: 'column', gap: isMobile ? 10 : 12 }}>
          {children.length > 0 ? children.map((child, i) => {
            ...
            return (
              <div key={child.id || i} style={{ display: 'flex', alignItems: 'center', gap: isMobile ? 12 : 14, padding: isMobile ? '12px' : '14px 16px', borderRadius: 12, background: '#f5f5f7', transition: 'all 0.2s', cursor: 'default' }}>
                ...
                <div style={{ flex: 1, minWidth: 0 }}>
                  <div style={{ fontSize: isMobile ? 14 : 15, fontWeight: 600, color: 'var(--color-text-primary)' }}>{child.name}</div>
                  <div style={{ fontSize: isMobile ? 12 : 13, color: 'var(--color-text-secondary)' }}>{child.grade || 'N/A'} · Last active: {child.last_active || 'Unknown'}</div>
                </div>
            ...
            <div style={{ fontSize: 14, color: 'var(--color-text-secondary)', textAlign: 'center', padding: 20 }}>No children connected yet</div>
          )}
        </div>
      </div>
    </DashboardLayout>
